function [w1, b1, w2, b2] = GNPSVM(Atrain,Btrain,FunPara) 
% Input:
...
if nargin == 2
    Rp = 2; Rq = 2; Rs = 2; Rdelta = 0.01; Rsigma = 0.01; w0 = ones(size(Atrain,2) + 1,1); itmax = 20; epsmax = 10^(-3); 
elseif nargin == 3
...
% % %     For A
    ...
    for i = 1:nSmpB
        ht_A = ht_A + abs(wp0'*Btrainbar(:,i))^(Rs-1)*sign(wp0'*Btrainbar(:,i))*Btrainbar(:,i);
    end
...
% % %     For B
    ...
    for i = 1:nSmpA
        ht_B = ht_B + abs(wn0'*Atrainbar(:,i))^(Rs-1)*sign(wn0'*Atrainbar(:,i))*Atrainbar(:,i);
    end
